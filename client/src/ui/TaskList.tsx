  ...
  return (
    <div className="flex flex-col justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 align-middle w-screen max-w-4xl mx-auto px-6 ">
      ...
      {(taskVM.tasks.length > 0) && <div className="bg-gray-700/80 backdrop-blur-md border border-white/10 rounded-2xl px-1.5 py-1">
        ...
        <div className="flex flex-col m-1 p-2 bg-gray-800/80 border-1 border-gray-500/30 rounded-xl">
          <div className="flex mx-1 pb-0 justify-between">
            <button className="flex hover:bg-white/10 rounded-lg" onClick={() => setShowDone(!showDone)}>
              ...
              <p className="text-gray-300 my-auto px-1.5">Show done</p>
            </button>
            <button className="text-red-500" onClick={taskVM.clearFinished}>Clear Done</button>
          </div>
          <div
