...
export function upsertTaskUpdateJob(queue: Job[], id: number, baseTask: TaskRead, patch: Partial<TaskCreate>): Job[] {
  ...
  return queue.map((j: Job, i: number) => {
...
export function upsertTaskDeleteJob(queue: Job[], id: number): Job[] {
  ...
  const job: Job = queue[jobPosition];

  switch (job.type) {
    case TaskAction.CREATE:
      return queue.filter((existingJob) => existingJob.payload.id !== id);
    case TaskAction.UPDATE:
      return queue.map((existingJob) =>
        existingJob.payload.id === id ? { type: TaskAction.DELETE, payload: { id } } : existingJob,
      );
